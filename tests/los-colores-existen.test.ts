 ...
 *
 * Acá había una sola: un `hover:text-tx-primary` en el botón de quitar un
 * destinatario. El token es `tx-main`; `tx-primary` no existió nunca, así que
 * ese botón no cambiaba de color al pasarle por encima y quedaba pareciendo
 * que no se podía tocar.
 *
 * Una sola clase no justifica un PR, pero la guardia sí: lo que este archivo
 * cuida no es ese botón, es que no vuelvan a entrar — y en las otras tres
 * aplicaciones del barrido eran 138, 15 y 6.
 *
...
describe('las clases de color', () => {
	...
	test('y los tokens que ya no existen no vuelven por su nombre viejo', () => {
		// El que había. Va por nombre además de por la regla general
